...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	if ((section == 0) && ([users2 count] != 0)) return @"Уже используют РЭУ:Chat";
	if ((section == 1) && ([users1 count] != 0)) return @"К сожалению, еще не используют РЭУ:Chat";
	return nil;
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	if (([user[@"emails"] count] != 0) && ([user[@"phones"] count] != 0))
	{
		UIActionSheet *action = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"Отмена"
											  destructiveButtonTitle:nil otherButtonTitles:@"Пригласить по почте", @"Пригласить по SMS", nil];
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	else [ProgressHUD showError:@"Пожалуйста настройте почту."];
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	{
		[ProgressHUD showSuccess:@"Письмо успешно отправлено."];
	}
	[self dismissViewControllerAnimated:YES completion:nil];
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	else [ProgressHUD showError:@"SMS не может быть отправлена."];
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	{
		[ProgressHUD showSuccess:@"SMS успешно отправлена."];
	}
	[self dismissViewControllerAnimated:YES completion:nil];
}
